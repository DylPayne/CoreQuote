...
def split_run_into_rows(desc: str, run_length: int, width: int, board_length_mm: int, board_type_id: int | None) -> list[dict]:
    ...
    rows: list[dict] = []
    if full > 0:
        rows.append({"Desc": desc, "L": int(board_length_mm), "W": int(width), "Qty": int(full), "board_type_id": board_type_id})
    if rem > 0:
